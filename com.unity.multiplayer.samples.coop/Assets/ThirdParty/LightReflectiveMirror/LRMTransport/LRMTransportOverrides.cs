...
namespace LightReflectiveMirror
{
    public partial class LightReflectiveMirrorTransport : Transport
    {
        ...
        // ClientSend/ServerSend below wrap every payload with opcode(1) + length-prefix(4) +
        // target-id(4) before handing it to clientToServerTransport. Both sizing methods must
        // reserve that overhead — otherwise Mirror batches messages right up to the inner
        // transport's raw max size, and the LRM wrapper pushes the wire size past it, corrupting
        // (or, via the unsafe pointer writes in LRMTools/DataHandler, overflowing) _clientSendBuffer.
        const int k_LrmOverhead = 9;

        public override int GetMaxPacketSize(int channelId = 0) =>
            clientToServerTransport.GetMaxPacketSize(channelId) - k_LrmOverhead;

        public override int GetBatchThreshold(int channelId = 0) =>
            clientToServerTransport.GetBatchThreshold(channelId) - k_LrmOverhead;
        ...
        public override void ClientSend(ArraySegment<byte> segment, int channelId)
        {
            ...
            else
            {
                ...
                _clientSendBuffer.WriteBytes(ref pos, segment);
