...
namespace LightReflectiveMirror
{
    ...
    public partial class LightReflectiveMirrorTransport : Transport
    {
        ...
        private void SetupCallbacks()
        {
            ...
            clientToServerTransport.OnClientError = (error, reason) => Debug.LogError($"[LRM] Client transport error {error}: {reason}");
        }
            ...
            try
            {
                ...
                switch (opcode)
                {
                    case OpCodes.Authenticated:
                        ...
                        // NOTE: RequestServerList() intentionally NOT called. It HTTP-GETs LRM's
                        // built-in server-list endpoint (serverIP:endpointServerPort, 8080), which is
                        // disabled on our relay (USE_ENDPOINT=false) — the request just hangs ~21s and
                        // logs "Network Error while retreiving the server list!" on every auth. This
                        // game lists rooms through the master server, not LRM's endpoint, so nothing
                        // reads relayServerList. Re-enable only if USE_ENDPOINT is turned on.
                        break;
