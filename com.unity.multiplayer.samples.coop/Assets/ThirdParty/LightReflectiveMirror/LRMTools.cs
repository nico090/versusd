...
namespace LightReflectiveMirror
{
    public static class LRMTools
    {
        ...
        // ArraySegment overload: writes the segment's payload (respecting Offset) with the same
        // length-prefixed wire format as the byte[] version, but without allocating a temp array
        // per send. Mirror's ClientSend/ServerSend hand us a segment into a pooled batch buffer whose
        // Offset is not guaranteed to be 0 — the old `segment.Array.Take(segment.Count).ToArray()`
        // copied from index 0 (wrong bytes if Offset != 0) AND allocated a byte[] on every packet.
        public static void WriteBytes(this byte[] data, ref int position, ArraySegment<byte> value)
        {
            data.WriteInt(ref position, value.Count);
            Buffer.BlockCopy(value.Array, value.Offset, data, position, value.Count);
