...
namespace Unity.BossRoom.Gameplay.Actions
{
    ...
    public class ReviveAction : Action
    {
        ...
        {
            if (m_Data.TargetIds == null || m_Data.TargetIds.Length == 0 || NetworkIdentityUtils.FindByNetId((uint)m_Data.TargetIds[0]) == null)
            {
                Debug.Log("Failed to start ReviveAction. The target entity  wasn't submitted or doesn't exist anymore");
                return false;
            }

            var targetNetworkObject = NetworkIdentityUtils.FindByNetId((uint)m_Data.TargetIds[0]);
