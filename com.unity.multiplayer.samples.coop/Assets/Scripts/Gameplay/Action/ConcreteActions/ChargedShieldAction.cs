...
namespace Unity.BossRoom.Gameplay.Actions
{
    ...
    public partial class ChargedShieldAction : Action
    {
        ...
        public override bool OnStart(ServerCharacter serverCharacter)
        {
            ...
            {
                var initialTargetIdentity = NetworkIdentityUtils.FindByNetId((uint)m_Data.TargetIds[0]);
                if (initialTargetIdentity)
                {
                    // face our target, if we had one
                    serverCharacter.physicsWrapper.Transform.LookAt(initialTargetIdentity.transform.position);
                }
            }
            ...
            serverCharacter.serverAnimationHandler.ResetTrigger(Config.Anim2);
        ...
        public override void Cancel(ServerCharacter serverCharacter)
        {
            ...
            {
                serverCharacter.serverAnimationHandler.SetAnimatorInt(Config.OtherAnimatorVariable,
                    serverCharacter.serverAnimationHandler.GetAnimatorInt(Config.OtherAnimatorVariable) - 1);
            }
        }
        ...
        private void StopChargingUp(ServerCharacter parent)
        {
            if (IsChargingUp())
            {
                ...
                parent.serverAnimationHandler.SetTrigger(Config.Anim2);

                parent.serverAnimationHandler.ResetTrigger(Config.Anim);
                ...
                if (Mathf.Approximately(GetPercentChargedUp(), 1f))
                {
                    ...
                    parent.serverAnimationHandler.SetAnimatorInt(Config.OtherAnimatorVariable,
                        parent.serverAnimationHandler.GetAnimatorInt(Config.OtherAnimatorVariable) + 1);
                }
            }
        }
