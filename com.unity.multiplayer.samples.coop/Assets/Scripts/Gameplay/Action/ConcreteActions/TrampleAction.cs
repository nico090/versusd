...
namespace Unity.BossRoom.Gameplay.Actions
{
    ...
    public partial class TrampleAction : Action
    {
        ...
        public override bool OnStart(ServerCharacter serverCharacter)
        {
            ...
            {
                var initialTarget = NetworkIdentityUtils.FindByNetId((uint)m_Data.TargetIds[0]);
                if (initialTarget)
                {
                    Vector3 lookAtPosition;
                    if (PhysicsWrapper.TryGetPhysicsWrapper((ulong)(uint)initialTarget.netId, out var physicsWrapper))
                    {
        ...
        private void CollideWithVictim(ServerCharacter parent, ServerCharacter victim)
        {
            ...
            // In PvP mode a PC can damage another PC; NPCs still only damage PCs.
            bool pvpPcVsPc = GameDataSource.IsPvPMode && !parent.IsNpc && !victim.IsNpc;
            if (parent.IsNpc != victim.IsNpc || pvpPcVsPc)
            {
                ...
                int damage;
                if (m_Data.TargetIds != null && m_Data.TargetIds.Length > 0 && m_Data.TargetIds[0] == (ulong)(uint)victim.GetComponent<NetworkIdentity>().netId)
                {
        ...
        private void StunSelf(ServerCharacter parent)
        {
            if (!m_WasStunned)
            {
                ...
                parent.clientCharacter.RpcCancelAllActions();
            }
            m_WasStunned = true;
        }
