...
namespace Unity.BossRoom.Gameplay.Actions
{
    ...
    public abstract class Action : ScriptableObject
    {
        ...
        public virtual bool OnStartClient(ClientCharacter clientCharacter)
        {
            ...
            // Play the action's animation on every client. Normally the trigger is fired
            // server-side through Mirror's NetworkAnimator, but on a headless dedicated server
            // that path never reaches clients: player avatars use a client-authority animator
            // (SetTrigger early-returns when !isClient) and NPCs have no NetworkAnimator at all.
            // OnStartClient runs on all clients (via RpcPlayAction), so we drive the swing/cast/
            // shot/trample/toss here instead. Charged actions are skipped — they manage their own
            // charge/stop animation timing, and firing only the start trigger would stick the pose.
            if (!string.IsNullOrEmpty(Config.Anim)
                && Config.Logic != ActionLogic.ChargedShield
                && Config.Logic != ActionLogic.ChargedLaunchProjectile
                && clientCharacter.OurAnimator)
            {
        ...
        public static bool ShouldClientAnticipate(ClientCharacter clientCharacter, ref ActionRequestData data)
        {
            ...
            if (data.ShouldClose == true)
            {
                ...
                var networkIdentity = NetworkIdentityUtils.FindByNetId((uint)targetId);
                if (networkIdentity != null)
                {
                    float rangeSquared = actionDescription.Range * actionDescription.Range;
                    isTargetEligible = (networkIdentity.transform.position - clientCharacter.transform.position).sqrMagnitude < rangeSquared;
                }
            }
