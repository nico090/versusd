...
namespace Unity.BossRoom.Gameplay.GameplayObjects.Character.AI
{
    public class AttackAIState : AIState
    {
        ...
        public override void Update()
        {
            ...
            float distToFoe = Vector3.Distance(
                m_Brain.GetMyServerCharacter().physicsWrapper.Transform.position,
                m_Foe.physicsWrapper.Transform.position);
            Debug.Log($"[AttackDebug] {m_Brain.GetMyServerCharacter().name} issuing attack '{m_CurAttackAction.name}' at foe {m_Foe.NetworkObjectId} (dist={distToFoe:F2}, attackRange={m_CurAttackAction.Config.Range})");
