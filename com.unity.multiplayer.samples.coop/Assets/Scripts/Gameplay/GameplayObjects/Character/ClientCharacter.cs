...
namespace Unity.BossRoom.Gameplay.GameplayObjects.Character
{
    ...
    public class ClientCharacter : NetworkBehaviour
    {
        ...
        [ClientRpc]
        public void RpcPlayAction(ActionRequestData data)
        {
        ...
        /// </summary>
        [ClientRpc]
        public void RpcCancelAllActions()
        {
        ...
        /// </summary>
        [ClientRpc]
        public void RpcCancelActionsByPrototypeID(ActionID actionPrototypeID)
        {
        ...
        /// </summary>
        [ClientRpc]
        public void RpcStopChargingUp(float percentCharged)
        {
            m_ClientActionViz.OnStoppedChargingUp(percentCharged);
        }

#if UNITY_EDITOR
        // These graphics prefabs are always nested at runtime under a Character root that has
        // NetworkIdentity. Suppress Mirror's base check when validating the standalone prefab
        // in isolation (no parent exists yet, so GetComponentInParent returns null).
        protected override void OnValidate()
        {
            if (GetComponentInParent<NetworkIdentity>(true) != null)
                base.OnValidate();
        }
#endif
        ...
        public override void OnStartClient()
        {
            ...
            {
                name = "AvatarGraphics" + (ulong)(uint)(m_ServerCharacter.connectionToClient?.connectionId ?? 0);
        ...
        void OnAnimEvent(string id)
        {
            ...
            // On a headless dedicated server the client-side action visualization is never
            // initialized, but the server still runs the Animator and fires its events — guard
            // against the NRE rather than dereferencing a null m_ClientActionViz.
            if (m_ClientActionViz == null) { return; }
