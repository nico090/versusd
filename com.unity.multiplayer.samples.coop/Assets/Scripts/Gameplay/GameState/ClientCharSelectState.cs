...
namespace Unity.BossRoom.Gameplay.GameState
{
    ...
    public class ClientCharSelectState : GameStateBehaviour
    {
        ...
        void OnNetworkSpawn()
        {
            // NetcodeHooks fires from both OnStartServer and OnStartClient — only init once as client.
            if (!m_NetcodeHooks.isClient || m_ClientInitialized)
                return;

            m_ClientInitialized = true;
            m_NetworkCharSelection.sessionPlayers.Callback += OnSessionPlayerStateChanged;
            Debug.Log($"[CharSelect] Client OnNetworkSpawn — sessionPlayers.Count={m_NetworkCharSelection.sessionPlayers.Count} netId={m_NetworkCharSelection.netId} localPlayerId={GetLocalPlayerId()}");

            // sessionPlayers may already contain data if the server seated players before we
            // subscribed to Callback (OnStartServer fires before OnStartClient, so the host's
            // seat is added before the client UI initializes). Refresh the UI manually.
            if (m_NetworkCharSelection.sessionPlayers.Count > 0)
            {
                UpdateSeats();
                UpdatePlayerCount();
                ...
                {
                    var p = m_NetworkCharSelection.sessionPlayers[i];
                    if (string.IsNullOrEmpty(localId) || p.PlayerId != localId) continue;

                    if (p.SeatState == NetworkCharSelection.SeatState.Inactive)
                    {
                        UpdateCharacterSelection(NetworkCharSelection.SeatState.Inactive);
                        OnAssignedPlayerNumber(p.PlayerNumber);
                    }
                    else
                    {
                        UpdateCharacterSelection(p.SeatState, p.SeatIdx);
                    }
                    break;
                }
            }
        ...
        /// <summary>
        /// Resolves the local player's stable master-server id (the value stored in
        /// <see cref="NetworkCharSelection.SessionPlayerState.PlayerId"/>). We match our own seat by this
        /// rather than by ClientId: Mirror never tells a remote client the server-assigned connectionId
        /// (LocalConnectionId is 0 on every client), so matching on ClientId always failed on a dedicated
        /// client and the player could never identify — or lock in — their seat. The PlayerId is known to
        /// both sides, so it works for host and remote clients alike.
        /// </summary>
        static string GetLocalPlayerId()
        {
            return ClientAuthPayload.Current?.PlayerId;
        }

        /// <summary>
        /// Called by the server when any of the seats in the session have changed. (Including ours!)
        /// </summary>
        void OnSessionPlayerStateChanged(SyncList<NetworkCharSelection.SessionPlayerState>.Operation op, int index, NetworkCharSelection.SessionPlayerState oldItem, NetworkCharSelection.SessionPlayerState newItem)
        {
            UpdateSeats();
            UpdatePlayerCount();
            ...
            for (int i = 0; i < m_NetworkCharSelection.sessionPlayers.Count; ++i)
            {
                if (!string.IsNullOrEmpty(localPlayerId) &&
                    m_NetworkCharSelection.sessionPlayers[i].PlayerId == localPlayerId)
                {
                    localPlayerIdx = i;
                    break;
                }
        ...
        {
            Debug.Log($"[CharSelect] OnPlayerClickedSeat({seatIdx}) — netId={m_NetworkCharSelection?.netId} sessionPlayers={m_NetworkCharSelection?.sessionPlayers?.Count}");
            if (m_NetworkCharSelection == null) { Debug.LogError("[CharSelect] m_NetworkCharSelection is NULL"); return; }
            if (m_NetworkCharSelection.netId == 0) { Debug.LogError("[CharSelect] netId==0, object not spawned"); return; }
        ...
        {
            if (m_NetworkCharSelection.netId != 0)
            {
                // request to lock in or unlock if already locked in
                // The server resolves our identity from the command sender; no need to pass our id.
