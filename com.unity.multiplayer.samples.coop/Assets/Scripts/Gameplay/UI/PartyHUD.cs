...
namespace Unity.BossRoom.Gameplay.UI
{
    ...
    public class PartyHUD : MonoBehaviour
    {
        ...
        void PlayerAvatarRemoved(ClientPlayerAvatar clientPlayerAvatar)
        {
            ...
            {
                ulong netId = (ulong)clientPlayerAvatar.GetComponent<NetworkIdentity>().netId;
                if (m_TrackedAllies.ContainsKey(netId))
                {
                    RemoveAlly(netId);
                    m_TrackedAllies.Remove(netId);
                }
            }
        }
        ...
        void SetHeroData(ClientPlayerAvatar clientPlayerAvatar)
        {
            ...
            m_PartyIds[0] = (ulong)m_OwnedServerCharacter.GetComponent<NetworkIdentity>().netId;
            ...
            m_OwnedServerCharacter.NetHealthState.HitPointsChanged += SetHeroHealth;

#if UNITY_EDITOR || DEVELOPMENT_BUILD
            m_OwnedServerCharacter.NetLifeState.IsGodModeChanged += SetHeroGodModeStatus;
#endif

            // plus we track their target
            m_OwnedServerCharacter.TargetIdChanged += OnHeroSelectionChanged;
        ...
        /// <summary>
        /// Gets Player Name from the NetworkIdentity of his controlled Character.
        /// </summary>
        ...
        void SetAllyData(ClientPlayerAvatar clientPlayerAvatar)
        {
            ...
            ulong id = (ulong)serverCharacter.GetComponent<NetworkIdentity>().netId;
            ...
            serverCharacter.NetHealthState.HitPointsChanged += (int previousValue, int newValue) =>
            {
                SetAllyHealth(id, newValue);
            };

#if UNITY_EDITOR || DEVELOPMENT_BUILD
            serverCharacter.NetLifeState.IsGodModeChanged += (value, newValue) =>
            {
                SetAllyGodModeStatus(id, newValue);
            };
#endif

            m_TrackedAllies.Add(id, serverCharacter);
        }
...
#if UNITY_EDITOR || DEVELOPMENT_BUILD
            m_PartyHealthGodModeImages[slot].gameObject.SetActive(serverCharacter.NetLifeState.IsGodMode);
#endif
        ...
        /// </summary>
        /// <param name="id"> NetworkIdentity netId of the ally. </param>
