...
namespace Unity.BossRoom.Navigation
{
    public sealed class DynamicNavPath : IDisposable
    {
        ...
        void RecalculatePath()
        {
            ...
            bool ok = m_Agent.CalculatePath(TargetPosition, m_NavMeshPath);
            Debug.Log($"[MoveDebug] CalculatePath target={TargetPosition} ok={ok} status={m_NavMeshPath.status} corners={m_NavMeshPath.corners.Length} agentOnMesh={m_Agent.isOnNavMesh}");
