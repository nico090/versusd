...
namespace Unity.Multiplayer.Samples.Utilities
{
    ...
    public class ClientLoadingScreen : MonoBehaviour
    {
        ...
        void OnProgressTrackersUpdated()
        {
            ...
            // Add progress bars for clients that are now tracked. Skip our own tracker via
            // NetworkBehaviour.isOwned — Mirror never tells a remote client its server-assigned
            // connectionId (LocalConnectionId is 0 everywhere), so comparing ids would fail on a
            // dedicated client. isOwned is reliable: a client always knows which objects it owns.
            foreach (var progressTracker in m_LoadingProgressManager.ProgressTrackers)
            {
                var clientId = progressTracker.Key;
                if (!progressTracker.Value.isOwned && !m_LoadingProgressBars.ContainsKey(clientId))
                {
        ...
        void ReinitializeProgressBars()
        {
            ...
            {
                var clientId = progressTracker.Key;
                if (!progressTracker.Value.isOwned)
                {
                    // The dictionary is normally populated via OnProgressTrackersUpdated, but the
                    // ordering with StartLoadingScreen isn't guaranteed — add on demand so we never
                    // index a missing entry.
                    if (!m_LoadingProgressBars.ContainsKey(clientId))
                    {
                        AddOtherPlayerProgressBar(clientId, progressTracker.Value);
                    }
                    else
