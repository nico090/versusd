...
namespace LightReflectiveMirror
{
    public partial class RelayHandler
    {
        ...
        {
            // TryGetValue, not the indexer: a client's first unreliable-channel send can arrive
            // before its CreateRoom/JoinServer (reliable channel) is processed — the two channels
            // have no ordering guarantee relative to each other. That raced packet has nowhere to
            // go yet; drop it instead of throwing KeyNotFoundException (was the real cause behind
            // the old "sent bad data" disconnects, see VersusD project memory "relay-lrm-migration").
            if (!_cachedClientRooms.TryGetValue(clientId, out Room room) || room == null)
            {
                return;
            }
