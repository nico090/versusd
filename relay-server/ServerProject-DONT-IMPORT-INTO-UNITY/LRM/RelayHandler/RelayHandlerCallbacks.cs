    ...
    public partial class RelayHandler
    {
        ...
        public void HandleMessage(int clientId, ArraySegment<byte> segmentData, int channel)
        {
            try
            {
                ...
                switch (opcode)
                {
                    ...
                    case OpCodes.UpdateRoomData:
                        if (!_cachedClientRooms.TryGetValue(clientId, out var plyRoom) || plyRoom == null || plyRoom.hostId != clientId)
                            return;
            ...
            catch (Exception e)
            {
                ...
                // TEMP diagnostic logging (2026-07-21): the blanket catch used to swallow the
                // real exception, leaving "sent bad data" as the only clue. Logging the full
                // exception + the raw segment shape (Offset/Count/backing-array Length) to find
                // the actual parse failure (see VersusD project memory "relay-lrm-migration").
                Program.WriteLogMessage(
                    $"Client {clientId} sent bad data! segment(Offset={segmentData.Offset}, Count={segmentData.Count}, " +
                    $"ArrayLen={segmentData.Array?.Length}) channel={channel} ex={e.GetType().Name}: {e.Message}\n{e.StackTrace}");
                Program.transport.ServerDisconnect(clientId);
            }
        }
